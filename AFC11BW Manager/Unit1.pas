...
const
 StockCars=66;
 MaxCars=256;
 VersionInfo='AFC11BW Manager       Version 0.1c (21 Apr 2009)';
...
Form2.Show;
Form2.Repaint;
if fileexists('krom.dev') then ChDir('E:\AFC11 - Crash Time 2');
RootDir:=getcurrentdir;
if not fileexists('FrontEnd2\FrontEnd.ds') then begin
Form2.FormStyle:=fsNormal;
MessageBox(Form1.Handle,'"FrontEnd2\FrontEnd.ds" not found. Run BWMan from AFC11BW folder.','Error',MB_OK);
Form1.Close;
exit;
end;
...
Value[6,12,ID].Int:=(ID-40)-1;                                 //MotorID
Value[6,13,ID].Int:=(ID-11)-1;                                 //
Value[6,14,ID].Int:=(ID*2-76)-1;                               //
Value[6,15,ID].Int:=(ID*2-75)-1;                               //
...
Value[8,2,(ID-40)].Int:=(ID-40)-1;             //
Value[8,3,(ID-40)].Int:=EC_Value[i,2,50,2].Int;//
Value[8,4,(ID-40)].Rel:=EC_Value[i,2,51,2].Rel;//
Value[8,5,(ID-40)].Rel:=EC_Value[i,2,52,2].Rel;//
Value[8,6,(ID-40)].Rel:=EC_Value[i,2,53,2].Rel;//
Value[8,7,(ID-40)].Rel:=EC_Value[i,2,54,2].Rel;//
Value[8,8,(ID-40)].Rel:=EC_Value[i,2,55,2].Rel;//
Value[8,9,(ID-40)].Rel:=EC_Value[i,2,56,2].Rel;//
Value[8,10,(ID-40)].Rel:=EC_Value[i,2,57,2].Rel;//
Value[8,11,(ID-40)].Rel:=EC_Value[i,2,58,2].Rel;//
Value[8,12,(ID-40)].Rel:=EC_Value[i,2,59,2].Rel;//
Value[8,13,(ID-40)].Rel:=EC_Value[i,2,60,2].Rel;//
Value[8,14,(ID-40)].Rel:=EC_Value[i,2,61,2].Rel;//
Value[8,15,(ID-40)].Rel:=EC_Value[i,2,62,2].Rel;//
Value[8,16,(ID-40)].Rel:=EC_Value[i,2,63,2].Rel;//
Value[8,17,(ID-40)].Rel:=EC_Value[i,2,64,2].Rel;//
Value[8,18,(ID-40)].Rel:=EC_Value[i,2,65,2].Rel;//
Value[8,19,(ID-40)].Rel:=EC_Value[i,2,66,2].Rel;//
Value[8,20,(ID-40)].Rel:=EC_Value[i,2,67,2].Rel;//
Value[8,21,(ID-40)].Rel:=EC_Value[i,2,68,2].Rel;//
Value[8,22,(ID-40)].Rel:=EC_Value[i,2,69,2].Rel;//
Value[8,23,(ID-40)].Rel:=EC_Value[i,2,70,2].Rel;//
Value[8,24,(ID-40)].Rel:=EC_Value[i,2,71,2].Rel;//
Value[8,25,(ID-40)].Rel:=EC_Value[i,2,72,2].Rel;//NMStep
Value[8,26,(ID-40)].Rel:=0;//10500rpm
Value[8,27,(ID-40)].Rel:=0;//11rpm
Value[8,28,(ID-40)].Int:=EC_Value[i,2,75,2].Int;//SampleRate
//Value[8,29,(ID-40)].Int:=EC_Value[i,2,76,2].Int;//MotorSample
Value[8,30,(ID-40)].Int:=EC_Value[i,2,77,2].Int;//
Value[8,31,(ID-40)].Int:=EC_Value[i,2,78,2].Int;//
Value[8,32,(ID-40)].Str:=EC_Value[i,2,79,2].Str;//
Value[8,33,(ID-40)].Str:=EC_Value[i,2,80,2].Str;//
Value[8,34,(ID-40)].Int:=EC_Value[i,2,81,2].Int;//
Value[8,35,(ID-40)].Str:=EC_Value[i,2,82,2].Str;//
Value[8,36,(ID-40)].Int:=EC_Value[i,2,84,2].Int;//Lautstaerke
Value[8,37,(ID-40)].Int:=EC_Value[i,2,101,2].Int;//
Value[8,38,(ID-40)].Int:=EC_Value[i,2,102,2].Int;//
Value[8,39,(ID-40)].Int:=EC_Value[i,2,103,2].Int;//
Value[8,40,(ID-40)].Int:=EC_Value[i,2,104,2].Int;//
Value[8,41,(ID-40)].Int:=EC_Value[i,2,100,2].Int;//Drehzahlmesser
Value[8,42,(ID-40)].Rel:=0;//11.5rpm
Value[8,43,(ID-40)].Rel:=0;//12rpm
Value[8,44,(ID-40)].Rel:=0;//12.5rpm
Value[8,45,(ID-40)].Rel:=0;//13rpm
Value[8,46,(ID-40)].Rel:=0;//13.5rpm
Value[8,47,(ID-40)].Rel:=0;//14rpm
Value[8,48,(ID-40)].Rel:=0;//14.5rpm
Value[8,49,(ID-40)].Rel:=0;//15rpm

//GearboxID
//Value[9,1,ID].Str:= empty
Value[9,2,(ID-11)].Int:=(ID-11)-1;//
Value[9,3,(ID-11)].Int:=EC_Value[i,2,85,2].Int;//
Value[9,4,(ID-11)].Rel:=EC_Value[i,2,86,2].Rel;//1
Value[9,5,(ID-11)].Rel:=EC_Value[i,2,87,2].Rel;//2
Value[9,6,(ID-11)].Rel:=EC_Value[i,2,88,2].Rel;//3
Value[9,7,(ID-11)].Rel:=EC_Value[i,2,89,2].Rel;//4
Value[9,8,(ID-11)].Rel:=EC_Value[i,2,90,2].Rel;//5
Value[9,9,(ID-11)].Rel:=EC_Value[i,2,91,2].Rel;//6
Value[9,10,(ID-11)].Rel:=EC_Value[i,2,92,2].Rel;//7
Value[9,11,(ID-11)].Rel:=EC_Value[i,2,93,2].Rel;//RW

//Tires
for j:=0 to 1 do begin
//Value[10,1,ID].Str:= empty
Value[10,2,(ID*2+j-76)].Int:=(ID*2+j-76)-1;//
Value[10,3,(ID*2+j-76)].Rel:=EC_Value[i,2,94+j*3,2].Rel;//
Value[10,4,(ID*2+j-76)].Rel:=EC_Value[i,2,95+j*3,2].Rel;//
Value[10,5,(ID*2+j-76)].Rel:=EC_Value[i,2,96+j*3,2].Rel;//
end;    //
...
begin
AboutForm.Show(VersionInfo,'Manages AFC11BW addons.','AFC11BWMan');
end;
...
Form1.Close;
ChDir(RootDir);
ShellExecute(handle, 'open', 'BurningWheels.exe', nil, nil, SW_SHOWNORMAL);
end;
