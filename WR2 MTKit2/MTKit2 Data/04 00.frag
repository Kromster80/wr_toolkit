//5*3+1+11 = 27
  ...
  void main(void)
  {
    ...
    if ( Mat_ReflFres == 0.0 ) //Calculate Fresnel value if material requires it
...
//	     _Diff  = mix( _Diff , _Dirt.rgb , _Dirt.a );
    ...
    vec3 Color = mix( (Mat_Diff+Mat_Spec2) * Env_Ambi , Mat_Diff * 2.0 , Env_Diff ); //Apply diffuse solution
         Color = mix( Color , Mat_Tex.rgb * ( mix(Env_Ambi / 2.0 , 1.0 , Env_Diff) ) , Mat_Tex.a ); //Apply diffuse texture
