...
i:=0; //setlength(Runtime,28000); //20542- is default
repeat
inc(i);
readln(ft,Runtime[i]);
until(eof(ft));
...
inc(h); AddRuntime[AddQty].Lines[h]:=':_krScnBG'+int2fix(i,2);
end; AddRuntime[AddQty].Length:=h;
...
for k:=1 to AddonScenery[i].TrackQty do begin inc(h);
