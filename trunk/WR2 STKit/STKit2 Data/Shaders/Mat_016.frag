  ...
  void main(void)
  {
    ...
    vec3 Ln = gl_LightSource[0].position.xyz; 		// Normalized in render code
     ...
     float a = smoothstep (0.4375, 0.5625, kBlend.r)*Lay0.a*Lay3.a;
     float c = smoothstep (0.4375, 0.5625, kBlend.b)*Lay2.a;
     float ShadowCol = smoothstep (0.4375, 0.5625, kColor.a);
     float Diff = max(0.,dot(Nn,Ln))*1.8;

     vec3 TexColor =(Lay0.rgb*(1.-c)+Lay2.rgb*c)*(1.-a)+Lay3.rgb*a;
