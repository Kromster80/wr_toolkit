...
MessageBox(0,'TGA size must be 8,16,32...1024 x 24/32 bit','Error',mb_ok);
closefile(ftga); end
else begin
...
if UpperCase(tganame)='*.TGA' then begin
      ...
      FindClose(SearchRec);
MessageBox(0,@('Converted '+inttostr(ii)+' TGA files.')[1],'TGA2PTX v2',mb_ok);
end else
